...
describe("inline comments (REST)", () => {
  ...
  it("forbids resolving someone else's comment unless you're a manager", async () => {
    ...
    await prisma.permission.create({
      data: { workspaceId: s.ws.id, userId: other.user.id, resourceType: "document", resourceId: s.docId, role: "viewer" },
    });
    const create = await req({
      method: "POST",
      url: `/api/documents/${s.docId}/comments`,
    ...
    await prisma.permission.create({
      data: { workspaceId: s.ws.id, userId: peer.user.id, resourceType: "document", resourceId: s.docId, role: "viewer" },
    });
