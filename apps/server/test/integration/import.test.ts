...
describe("server-side vault import", () => {
  ...
  it("strips NUL bytes so notes with embedded 0x00 import instead of failing the DB write", async () => {
    const s = await baseScenario();
    const zip = Buffer.from(
      zipSync({
        "vault/Dirty.md": strToU8("# Title\u0000 with null\n\nBody\u0000 here.\n"),
      }),
    );
    const started = await uploadZip(s.adminCookie, s.ws.id, zip);
    expect(started.statusCode).toBe(202);
    const job = await waitForJob(s.adminCookie, started.json().jobId);
    expect(job.status).toBe("done");
    expect(job.report.documentsCreated).toBe(1);
    const doc = await prisma.document.findFirstOrThrow({ where: { workspaceId: s.ws.id, path: "imported/dirty.md" } });
    expect(doc.title).not.toContain("\u0000");
    expect(doc.searchText ?? "").not.toContain("\u0000");
  });
