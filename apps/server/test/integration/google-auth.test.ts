...
describe("google oauth", () => {
  it("reports config and 404s the routes when not configured", async () => {
    ...
    expect((await req({ method: "GET", url: "/api/auth/config" })).json()).toEqual({ googleEnabled: false, captcha: null, cloudHosted: false });
  ...
  it("start sets state + verifier cookies and redirects to Google", async () => {
    useGoogle();
    expect((await req({ method: "GET", url: "/api/auth/config" })).json()).toEqual({ googleEnabled: true, captcha: null, cloudHosted: false });
    const start = await req({ method: "GET", url: "/api/auth/google/start" });
