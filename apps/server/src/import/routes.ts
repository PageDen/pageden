// Server-side vault import endpoints.
//
...
export async function registerImportRoutes(app: FastifyInstance): Promise<void> {
  await app.register(async (instance) => {
    ...
    instance.post<{
      ...
      "/api/import/vault",
      {
        ...
        config: { rateLimit: { max: Number(process.env.IMPORT_UPLOAD_RATE_LIMIT_MAX ?? 30), timeWindow: "1 minute" } },
      },
      async (request, reply) => {
  ...
  app.get<{ Params: { id: string } }>(
    "/api/import/jobs/:id",
    { config: { rateLimit: { max: Number(process.env.IMPORT_POLL_RATE_LIMIT_MAX ?? 120), timeWindow: "1 minute" } } },
    async (request, reply) => {
      const auth = await requireAuth(request);
  ...
  app.post<{ Params: { id: string } }>(
    ...
    { config: { rateLimit: { max: Number(process.env.IMPORT_RETRY_RATE_LIMIT_MAX ?? 10), timeWindow: "1 minute" } } },
    async (request, reply) => {
      const auth = await requireAuth(request);
      const job = await prisma.importJob.findUnique({ where: { id: request.params.id } });
      if (!job) return notFound(reply, "Import job not found.");
