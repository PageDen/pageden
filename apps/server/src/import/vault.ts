// Server-side vault import worker.
//
...
// Postgres rejects NUL (0x00) in text/jsonb columns, and vault files occasionally carry
// stray nulls (binary contamination, odd editors). Strip them from anything we persist.
...
function normalizeMarkdown(value: string): string {
  const normalized = stripNul(value).replace(/\r\n?/g, "\n");
...
async function createDocumentInternal(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const doc = await tx.document.create({
      data: { workspaceId, folderId: folder.id, title: stripNul(title), slug, path, createdById: userId, updatedById: userId },
    });
